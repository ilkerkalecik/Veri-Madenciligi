...
def build_similarity_matrix_rule_based(
    ...
    """
    global_w[i,j] = tutan kuralların ağırlık toplamı (int)
    """
...
# ---------------------------------------------------------
# 3) KNN SKORU (rule-based global_w’dan)
# ---------------------------------------------------------
def knn_scores_for_candidate(global_w: np.ndarray, cand_idx: int, history: List[int], k: int):
    sims = global_w[cand_idx].astype(float)
    sims[cand_idx] = -1e9  # kendini ele
    ...
    knn_sim_score = float(np.mean(top_sims)) if len(top_sims) else 0.0

    hist_set = set(history)
    hits = sum(1 for i in top_idx if i in hist_set)
    knn_hit_rate = float(hits) / float(k_eff) if k_eff > 0 else 0.0

    sim_to_history_sum = float(global_w[cand_idx, history].sum())
    return knn_sim_score, knn_hit_rate, sim_to_history_sum, top_idx.tolist(), hits, k_eff

# ---------------------------------------------------------
# 4) TOP 10 TABLO PNG ÜRET (KNN)
# ---------------------------------------------------------
def save_top10_table_image_knn(res_df: pd.DataFrame, out_png: str,
                               algo_name: str, dataset_size: int, base_size: int, k_neighbors: int):
    cols = [
        "candidate_name", "candidate_artists",
        "knn_sim_score", "knn_hit_rate",
        "sim_to_first_base_sum", "final_score"
    ]
    top = res_df[cols].head(10).copy()

    top["knn_sim_score"] = top["knn_sim_score"].round(6)
    top["knn_hit_rate"] = top["knn_hit_rate"].round(6)
    ...
    title = f"{algo_name} | Dataset n={dataset_size} | Graph BASE_SIZE={base_size} | K={k_neighbors}\nTOP 10 ÖNERİ (KNN)"
...
# 5) GRAF ÇİZ (KNN)
# ---------------------------------------------------------
def draw_window_graph_knn(indices: List[int],
                          df: pd.DataFrame,
    ...
    for n in G.nodes:
        if G.nodes[n]["is_candidate"]:
            labels[n] = f'{G.nodes[n]["label"]}\n(CAND) hits:{hits_in_history}/{k_eff}'
        else:
...
def main():
    ...
    # skor ağırlıkları
    ALPHA = 0.70  # knn_sim_score
    BETA  = 0.30  # knn_hit_rate

    # graf çok kalabalıksa 4..7 deneyebilirsin (0=kapalı)
    EDGE_THRESHOLD = 7

    ALGO_NAME = "KNN (Rule-based Similarity) + Rolling Slot"
    name_col = "name"
    artist_col = "artists"
    ...
    # --- İstersen ağırlıklar (hepsi 1 bırakabilirsin) ---
    weights = {
        "danceability": 1.0,
        "energy": 1.0,
        "speechiness": 1.0,
    ...
    # numeric parse (kuralların ihtiyacı olanlar)
    needed_numeric = set(NUMERIC_COLS_RULE)
    ...
    results = []
    print(f"\n--- ADIM B: ({BASE_SIZE-1}) sabit + 1 slot ile KNN skorlama ---")
    pb = ProgressBar(len(candidates), prefix="Adaylar:   ")

    for cand_idx in candidates:
        knn_sim_score, knn_hit_rate, sim_to_history_sum, knn_list, hits, k_eff = knn_scores_for_candidate(
            global_w=global_w,
            cand_idx=cand_idx,
            history=history,
            k=K_NEIGHBORS
        ...
        final_score = ALPHA * knn_sim_score + BETA * knn_hit_rate
        ...
        results.append({
            ...
            "knn_k": K_NEIGHBORS,
            "knn_sim_score": knn_sim_score,
            "knn_hit_rate": knn_hit_rate,
            "sim_to_first_base_sum": sim_to_history_sum,
            "final_score": final_score,
            "knn_neighbor_indices": ",".join(map(str, knn_list)),
            "_hits": hits,
            "_k_eff": k_eff
        })
        pb.update()
    ...
    out_csv = f"{base_filename}_knn_rulebased_base{BASE_SIZE}_rolling_recommendations.csv"
    res_df.drop(columns=["_hits", "_k_eff"], errors="ignore").to_csv(out_csv, index=False, encoding="utf-8-sig")
    print(f"\n✅ KNN sonuçları kaydedildi: {out_csv}")
    ...
    out_dir = os.path.join(base_dir, f"{base_filename}_knn_rulebased_base{BASE_SIZE}_top{top_k}_graphs_{n}_size")
    os.makedirs(out_dir, exist_ok=True)

    # 1) TOP10 tablo PNG
    table_png = os.path.join(out_dir, f"TOP_{top_k:02d}_TABLE.png")
    save_top10_table_image_knn(
        res_df=res_df,
        out_png=table_png,
        algo_name=ALGO_NAME,
        dataset_size=n,
    ...
    for rank in range(top_k):
        ...
        hits = int(res_df.loc[rank, "_hits"])
        k_eff = int(res_df.loc[rank, "_k_eff"])
        ...
        draw_window_graph_knn(
            indices=window_indices,
            df=df,
            weight_mat=w_window,
            name_col=name_col,
            cand_idx=cand_idx,
            hits_in_history=hits,
            k_eff=k_eff,
            out_png=out_png,
            algo_name=ALGO_NAME,
            dataset_size=n,
    ...
    print("\n--- TOP 10 ÖNERİ (KNN / Rule-based Similarity) ---")
    show_cols = [
        "candidate_name", "candidate_artists",
        "knn_sim_score", "knn_hit_rate",
        "sim_to_first_base_sum", "final_score"
